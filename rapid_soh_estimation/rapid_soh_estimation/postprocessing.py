...
def create_cc_modeling_data(dir_preprocessed_data:Path, segment_length:int=600, segment_overlap:float=0.5, segment_soc_bounds=(0.3,0.9)) -> dict:
	...
	for cell_id in df_test_tracker['Cell ID'].unique():
		...
		for rpt_num,df_split in df_cc.sort_values(by=['RPT Number', 'Time (s)'], ascending=[True, True]).groupby('RPT Number'):
			...
			for b in segment_soc_bounds: assert b >= 0.0 and b <= 1.0
			# lower voltage limit per segment_soc_bounds[1] (inclusive)
			vlim_lower = df_v_vs_soc_1c_chg.loc[df_v_vs_soc_1c_chg['soc'] <= segment_soc_bounds[0]*100, 'v'].values[-1]
			# upper voltage limit per segment_soc_bounds[1] (exclusive)
			vlim_upper = df_v_vs_soc_1c_chg.loc[df_v_vs_soc_1c_chg['soc'] < segment_soc_bounds[1]*100, 'v'].values[-1]
...
def save_modeling_data(data_type:str, data:dict, filename:str=None) -> Path:
	...
	if not dir_processed_data.joinpath("LFP", data_type).exists():
...
def perform_postprocessing(dir_preprocessed_data:Path):
	...
	cc_data = create_cc_modeling_data(dir_preprocessed_data=dir_preprocessed_data, 
									  segment_length=600, segment_overlap=0.5, segment_soc_bounds=(0.3,0.9))
...
def postprocess_cc_data(dir_preprocessed_data:Path, **kwargs):
	print("Creating dataset for CC-segment model...")
	...
	try:
		...
		filename = None
	cc_data = create_cc_modeling_data(dir_preprocessed_data=dir_preprocessed_data, **kwargs)

	x = save_modeling_data(data_type='cc', data=cc_data, filename=filename)
	print(f"CC-segment dataset saved to: {x}")

	

if __name__ == '__main__':
	perform_postprocessing(dir_preprocessed_data=dir_preprocessed_data)
